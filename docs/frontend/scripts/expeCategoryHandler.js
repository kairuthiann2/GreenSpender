  ...
  const updateDescriptionOptions = (category) => {
    ...
    try {
      ...
      expenseDescription.innerHTML =
        '<option value="" disabled selected>Select Expense Description</option>';

      // Get the list of descriptions based on selected category
      ...
      if (descriptions.length === 0) {
        console.warn(
          `Warning: no descriptions available for category: ${category}`
        );
      }
      ...
      console.error(
        "Error occured while updating descriptions:",
        error.message
      );
    }
  };
