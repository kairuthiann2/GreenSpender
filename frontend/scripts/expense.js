...
const fetchExpenses = async () => {
    try {
        ...
        const expenses = await apiCall(`/api/expenses/view_expense/${user_id}`, 'GET');
...
const addExpense = async () => {
    ...
    try {
        await apiCall('/api/expenses/add_expense/', 'POST', { amount, date, category, description });
...
const fetchAndPrefillExpense = async (id) => {
    try {
        ...
        const expense = await apiCall(`/api/expenses/edit_expense/${id}`, 'GET'); 
        var date = new Date(expense.date);

        const formattedDate = date.toISOString().split('T')[0];

        console.log('Fetched expensed data:', expense);
    ...
    if (window.location.pathname.endsWith('edit_expense.html')) {
        ...
        if (form) {
            form.addEventListener('submit', async (e) => {
                ...
                //console.log('Updating expense with ID:', expenseId, { category, description, amount, date });

                try {
        
                    await apiCall(`/api/expenses/edit_expense/${expenseId}`, 'PUT', { category, description, amount, date });

                    console.log('Expense updated successfully.');

                    // Redirect to the view_expense.html page after successful edit
                    window.location.href = './view_expense.html';
...
const deleteExpense = async (id) => {
    ...
    try {      
        await apiCall(`/api/expenses/delete_expense/${id}`, 'DELETE');
